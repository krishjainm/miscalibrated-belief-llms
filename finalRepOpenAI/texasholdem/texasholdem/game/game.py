...
class TexasHoldEm:
    ...
    def add_chat_message(self, player_id: int, message: str, timestamp: str = None, 
        ...
        # Optional runtime sanitizer (e.g., banned-phrase paraphraser)
        sanitizer = getattr(self, 'chat_message_sanitizer', None)
        if callable(sanitizer):
            try:
                message = sanitizer(message)
            except Exception:
                # Fail open: keep original message if sanitizer errors
                pass
